...
Std_ReturnType ccp2_pwm_set_duty (const float duty);
...
# 14 "./ECU_layer/ultrasonic/hal_ultasonic.h"
typedef struct{
    pin_config_t TR_PIN;
...
ccp1_t ccp1_={.CCP1_handler=((void*)0),.ccp1_mode=ccp1_pwm_mode_select,.ccp1_mode_variant=0x0E,.pin_config.port=PORTC_INDEX
,.pin_config.pin=pin2,.pin_config.direction=GPIO_OUTPUT,.timer2_postscaler_cfg=1,
.timer2_prescaler_cfg=1,.pwm_frequence=0};
...
Timer3_t Timer3_1={.Timer3_handler=((void*)0),.timer3_mode=0,.timer3_preload_value=0,
                   .timer3_prescaler_cfg=Prescale_value_div_1,.timer3_wr__reg_cfg=0};
...
mssp_i2c_t mssp_i2c={.i2c_clock=100000,.i2c_cfg.i2c_SMBus_control=0,.i2c_cfg.i2c_general_call=1
,.i2c_cfg.i2c_mode=0,.i2c_cfg.i2c_slave_address=0x60,.i2c_cfg.i2c_slew_rate=0,
.i2c_cfg.i2c_mode_cfg=0x06U,.I2C_DefaultInterruptHandler=mssp_I2C_DefaultInterruptHandler};
...
int main() {
    ...
    while(1)
    {
        ...
        if(i2c_data_3 == 'Q'){
            led_turn_on(&led3);
        }else if(i2c_data_3 == 'W'){
            led_turn_off(&led3);
        }else if(i2c_data_3 == 'E'){
            led_turn_on(&led5);;
        }
        else if(i2c_data_3 == 'R'){
            led_turn_off(&led5);
        }
        else if(i2c_data_3 == 'T'){
            morot_turn_right(&motor_2);
        }
        else if(i2c_data_3 == 'Y'){
            morot_stop(&motor_2);
        }
        else if(i2c_data_3 == 'U'){
            morot_turn_right(&motor_1);
        }
        else if(i2c_data_3 == 'I'){
            morot_stop(&motor_1);
        }
        else if(i2c_data_3 == 'O'){
            GPIO_pIN_WRITE_LOGIC(&relay,GPIO_HIGH);
        }
        ...
        }else if(i2c_data_3 == 'V'){
            lcd_4bit_send_command(&lcd_4bit,0x08);
            led_turn_off(&led3);
            led_turn_off(&led5);
            GPIO_pIN_WRITE_LOGIC(&relay,GPIO_LOW);
            morot_stop(&motor_1);
            ...
            lcd_4bit_send_command(&lcd_4bit,0x0C);
            led_turn_on(&led3);
            led_turn_on(&led5);
            GPIO_pIN_WRITE_LOGIC(&relay,GPIO_HIGH);
            morot_turn_right(&motor_1);
